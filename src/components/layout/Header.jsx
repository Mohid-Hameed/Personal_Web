...
export default function Header({ title = "Portfolio", logoUrl }) {
  ...
  const navItems = NAV_VISIBLE_IDS.filter((id) => NAV_SECTIONS[id]).map(
    (id) => ({ id, label: NAV_SECTIONS[id] }),
  );
          ...
          <List disablePadding>
            {navItems.map(({ id, label }) => (
              <ListItem key={id} disablePadding>
                <ListItemButton
                  onClick={() =>
                    scrollToSection(id, () => setDrawerOpen(false))
                  }
                  sx={{
                    py: 1.5,
                    px: 2,
